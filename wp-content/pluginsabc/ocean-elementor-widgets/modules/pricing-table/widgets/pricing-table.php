	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .oew-pricing-table' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
			...
			if ( ! empty( $features ) ) :
				...
				<ul class="oew-pricing-table-list">
					...
					foreach ( $features as $index => $item ) :
						...
						$this->add_render_attribute( $key, 'class', 'oew-pricing-table-item-' . $item['_id'] );
			...
			if ( ! _.isEmpty( settings.items ) ) { #>
				<ul class="oew-pricing-table-list">
					<# _.each( settings.items, function( item, index ) {
						...
						view.addRenderAttribute( key, 'class', 'oew-pricing-table-item-' + item._id );
